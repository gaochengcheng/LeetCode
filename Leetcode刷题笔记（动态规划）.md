...
## S.book_Max length Increment Sub String（最长递增子序列）
...
## S.book_Max lengthest common sub sequence(最长公共子序列)

## S.book_Max lengthest common sub string(最长公共子串问题213页) 
...
1. 产生dp动态规划表，`dp[i][j]` 表示的含义是在必须把`str1[i]`和`str2[j]`当作公共子串最后一个字符的情况下，公共子串最长能有多长。
2. 根据动态规划表，找到其中的最大值，最大值就是子串的长度，然后根据最大值下标，在原数组`str1`或者`str2`中找到最长子串。
